...
## Purpose
This repository should be developed as a **fact-first DART disclosure analysis system**.

The goal is **not** to generate vague AI summaries.
The goal is to build a system that:
- collects DART disclosures reliably,
- extracts structured facts,
- evaluates them with explicit rules,
- produces evidence-based Korean reports for human review.
...
- You may inspect files, think, and implement in English internally.
- However, **every user-facing explanation, progress report, implementation summary, diagnosis, and final result shown to the user must be written in Korean**.
- Do not reply to the user in English unless explicitly requested.
...
1. **Do not fabricate anything.**
   - Do not invent repository state, file contents, code behavior, API behavior, execution results, schema, or parsed filing facts.
   - If something was not directly verified, state that clearly.
...
3. **Rule-based judgment first, wording last.**
   - The core system should still make sense without an LLM.
   - Narrative phrasing is optional; extraction and evaluation are mandatory.

4. **Be practical and implementation-focused.**
   - Prefer concrete code, inspection, diagnosis, and verifiable progress over abstract planning.
   - Do not give vague strategy-only answers when implementation is possible.

5. **Preserve working behavior.**
   - Reuse existing code if it exists.
   - Avoid breaking working behavior unless a change is explicitly required.

6. **Never reveal secrets.**
   - Never print the value of `DART_API_KEY` or any other secret.

7. **Keep the pipeline maintainable.**
   - Separate collection, normalization, extraction, evaluation, and reporting.
   - Avoid tightly coupling raw API responses directly to final report text.

---

## Required Progress Report Format
Every meaningful progress update shown to the user must be written in Korean using this structure:
...
Before major code changes, briefly explain the intended change in Korean.
After changes, clearly state in Korean:
- which files were created,
- which files were modified,
- what each file does,
- how to run the current result,
...
The initial product should **not** start with investment recommendations such as “buy” or “sell.”
...
The exact schema may evolve, but conceptually support the following entities.
...
Start with a practical subset. The system does not need to support every filing category on day one.
...
Do not try to normalize everything at once.
Start with categories that materially affect risk assessment.
...
Important:
A contract filing alone is **not automatically positive**.
The report should note that execution and later financial reflection still need verification.
...
Do not start with a black-box score.
Start with explicit rule-based flags and then optionally aggregate them.
...
Use explainable evaluation objects such as:
- `flag`: true / false
...
5. **No mixing unavailable values with inferred values without labels.**
   ...
   - Interpretation must be labeled as interpretation.
...
Main point:
Keep API client, extraction logic, evaluation logic, and report formatting separate.

---

## Minimum Quality Standard
When implementing based on this repository guide:
